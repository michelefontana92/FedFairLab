...
def determine_layers_gen(side, random_dim, num_channels):
    ...
    for prev, curr in zip(reversed(layer_dims), reversed(layer_dims[:-1])):
        layers_G += [
            BatchNorm2d(prev[0]),
            ReLU(True),
            ConvTranspose2d(prev[0], curr[0], 4, 2, 1,
...
class CTABGANSynthesizer:
    ...
    def fit(self, train_data=pd.DataFrame, categorical=[], mixed={}, type={}):
        ...
        optimizer_params = dict(lr=2e-4, betas=(0.5, 0.9),
                                eps=1e-3, weight_decay=self.l2scale)
        ...
        if target_index != None:
            ...
            classifier = Classifier(
                data_dim, self.class_dim, st_ed).to(self.device)
        ...
        for i in tqdm(range(self.epochs)):
            for _ in range(steps_per_epoch):
                ...
                loss_mean = torch.norm(torch.mean(info_fake.view(
                    self.batch_size, -1), dim=0) - torch.mean(info_real.view(self.batch_size, -1), dim=0), 1)
                loss_std = torch.norm(torch.std(info_fake.view(
                    self.batch_size, -1), dim=0) - torch.std(info_real.view(self.batch_size, -1), dim=0), 1)
