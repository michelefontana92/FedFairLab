    ...
    def fit(self):
        ...
        for id_, info in enumerate(self.meta):
            if info['type'] == "continuous":
                ...
                mode_freq = (pd.Series(gm.predict(
                    data[:, id_].reshape([-1, 1]))).value_counts().keys())
                model.append(gm)
                ...
                mode_freq = (pd.Series(gm2.predict(
                    data[:, id_][filter_arr].reshape([-1, 1]))).value_counts().keys())
    ...
    def inverse_transform(self, data):
        ...
        for id_, info in enumerate(self.meta):
            if info['type'] == "continuous":
                ...
                u = data[:, st]
                full_v = data[:, (st+1):(st+1)+len(info['modal']
...
class ImageTransformer():
    ...
    def transform(self, data):
        ...
        if self.height * self.height > len(data[0]):
            ...
            padding = torch.zeros(
                (len(data), self.height * self.height - len(data[0]))).to(data.device)
