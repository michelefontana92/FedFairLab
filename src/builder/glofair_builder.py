...
class GLOFAIR_Builder(Base_Builder):
    """
    ...
    def __init__(self,**kwargs):
        super(GLOFAIR_Builder,self).__init__(**kwargs)
        ...
        self.lr = self._algorithm_config.get('lr',1e-4)
        ...
        s_id = self._server_config.get('id','fedfairlab_server')
        s_ckpt_dir = self._server_config.get('checkpoint_dir',f'checkpoints/{self._project}')
        s_ckpt_name = self._server_config.get('checkpoint_name','fedfairlab_global_model.h5')
        
        self.server = ServerFactory().create('server_fedfairlab',
        config=s_config,
        id = s_id,
        project = self._project,
        model = self.model,
